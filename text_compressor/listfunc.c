...
/* Return the key at the index location of the list without removing it. */
...
int list_clear(List *list){
    ...
    while(el != NULL){
        ...
        if (list->destroy_data != NULL) {
            int ret = list->destroy_data(curr->data);
            if (ret < 0) return -1;
        }
        if (list->destroy_key != NULL) {
            int ret = list->destroy_key(curr->key);
            if (ret < 0) return -1;
        }
        free(curr);
    }
